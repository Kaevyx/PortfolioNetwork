...
async function getPublishedPrivacyPolicy() {
...
async function getAllPrivacyVersions() {
  const supabase = await createClient();
...
export default async function PrivacyPage(
  ...
  if (searchParams?.v) {
    ...
    document = await getPublishedPrivacyPolicy();
  }

  // Get all versions for history
  const allVersions = await getAllPrivacyVersions();
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 md:p-12">
            ...
            <p className="text-gray-700 dark:text-gray-300">
              No privacy policy document has been published yet. Please check back later.
            </p>
          </div>
        </div>
      </div>
                ...
                {allVersions.map((version) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <a
                        href={`/privacy?v=${version.version}`}
