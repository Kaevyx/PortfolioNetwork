-- Remove severity from status incidents
-- Severity is no longer needed since component_status is directly set by admins

-- First, drop all triggers that depend on severity
...
-- Update get_component_status_from_incident to remove severity parameter
-- Drop all possible function signatures first (using CASCADE to handle dependencies)
DROP FUNCTION IF EXISTS get_component_status_from_incident CASCADE;

-- Recreate with new signature (no severity parameter)
...
-- Update the trigger function to remove severity
...
-- Update get_most_severe_component_status to remove severity references
...
BEGIN
  ...
  FOR incident_record IN
    SELECT status, impact_scope, component_status
...
-- Update update_component_statuses_from_all_incidents to remove severity
...
-- Recreate triggers without severity dependency
DROP TRIGGER IF EXISTS trigger_update_component_statuses_from_incident ON status_incidents;
...
-- Recreate the trigger for updating all component statuses
DROP TRIGGER IF EXISTS trigger_update_component_statuses_from_all_incidents ON status_incidents;
